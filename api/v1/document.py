...
@document_router.post("", response_model=SuccessResponse[DocumentResponse], status_code=status.HTTP_201_CREATED)
...
async def get_document(
    ...
    document = await Document.objects.get(id=document_id, user_id=user.id, deleted_at=None)
    if not document:
...
@document_router.patch("/{document_id}", response_model=SuccessResponse[DocumentResponse])
