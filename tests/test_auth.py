...
async def test_register_user_success(client: AsyncClient):
    payload = {"email": "testauth@example.com", "password": "Password123!"}
...
async def test_token_success(client: AsyncClient):
    ...
    login_params = {"email": "logintest@example.com", "password": "Password123!"}
...
async def test_token_invalid_password(client: AsyncClient):
    payload = {"email": "badpass@example.com", "password": "Password123!"}
    await client.post("/api/v1/auth/register", json=payload)

    login_params = {"email": "badpass@example.com", "password": "WrongPassword"}
    response = await client.post("/api/v1/auth/token", params=login_params)
    assert response.status_code == 401
    body = response.json()
    assert body["success"] is False
    assert body["error"]["code"] == "INVALID_USERNAME_OR_PASSWORD"


@pytest.mark.asyncio
async def test_token_user_not_found(client: AsyncClient):
    login_params = {"email": "notfound@example.com", "password": "Password123!"}
    response = await client.post("/api/v1/auth/token", params=login_params)
...
async def test_refresh_token_success(client: AsyncClient):
    ...
    login_params = {"email": "refresh@example.com", "password": "Password123!"}
    response = await client.post("/api/v1/auth/token", params=login_params)
...
async def test_refresh_token_invalid(client: AsyncClient):
    refresh_response = await client.post(
        "/api/v1/auth/refresh", params={"token": "invalid_format_token"}
    )
...
async def test_logout_success(client: AsyncClient):
    ...
    login_params = {"email": "logout@example.com", "password": "Password123!"}
    response = await client.post("/api/v1/auth/token", params=login_params)
    refresh_token = response.json()["data"]["refresh_token"]

    logout_response = await client.post(
        "/api/v1/auth/logout", params={"token": refresh_token}
    )
