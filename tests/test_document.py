...
async def test_list_documents_success(client: AsyncClient):
    ...
    await client.post("/api/v1/document", json={"title": "Doc 1", "content": "C1"}, headers=headers)
    await client.post("/api/v1/document", json={"title": "Doc 2", "content": "C2"}, headers=headers)
...
async def test_get_document_success(client: AsyncClient):
    ...
    create_resp = await client.post("/api/v1/document", json={"title": "Target", "content": "Find me"}, headers=headers)
...
async def test_update_document_success(client: AsyncClient):
    ...
    create_resp = await client.post("/api/v1/document", json={"title": "Old Title", "content": "Old Content"}, headers=headers)
    doc_id = create_resp.json()["data"]["id"]
    
    response = await client.patch(f"/api/v1/document/{doc_id}", json={"title": "New Title"}, headers=headers)
    assert response.status_code == 200
    body = response.json()
...
async def test_delete_document_soft(client: AsyncClient):
    ...
    create_resp = await client.post("/api/v1/document", json={"title": "To Delete", "content": "Bye"}, headers=headers)
    doc_id = create_resp.json()["data"]["id"]
